...
def create_dashboard(process, d ,doc):
	create_production_dashboard( process, d, doc)
...
def add_data_in_work_order_assignment(doc, method):
	...
	for d in doc.get('sales_invoice_items_one'):
		if cint(d.check_split_qty)==1:
			...
			for s in split_qty:
				if s:
					make_order(doc,d, split_qty[s]['qty'])
		else:
...
def make_order(doc, d, qty):
	if not frappe.db.get_value('Work Order Distribution', {'refer_doc':d.name},'refer_doc'):
		...
		e.tailor_qty = qty
		e.serial_no_data = generate_serial_no(doc, d.tailoring_item_code, qty)
...
def get_process_detail(name):
	...
	return frappe.db.sql("select process_data, process_name, ifnull(trials,'No') as trials, status from `tabProcess Log` where parent ='%s' and branch = '%s' order by idx"%(name, branch),as_dict=1, debug=1)